...
testo_CRP = read_excel("INPUT_Lille/Covid_CRP.xlsx") 

pdf("OUTPUT/Supp_1A_Testo_CRP_spearman.pdf", width = 6, height = 4.25)
ggplot(testo_CRP,aes(x=CRP, y=TESTOSTERONE, fill=GROUP_Norm_AB)) +
  geom_point(size = 3, shape = 21, alpha = 0.6) +
  geom_smooth(method=lm, na.rm = TRUE, fullrange= TRUE,
              aes(group=1),colour="black", se = FALSE) + stat_cor( method = "spearman",(aes(group=1))) + 
...
save.image("D:/PROJECTS/COVID_April 2020/Machine learning models clinical data/Clinical_data_Covid/Waljit/Manuscript/ebiomedicine_revisions/Lille_ICU_data.RData")
